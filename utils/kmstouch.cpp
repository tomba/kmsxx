...
static const char* usage_str =
		...
		"Options:\n"
		"      --ddevice=DEVICE      DEVICE is the path to DRM card to open\n"
		"      --idevice=DEVICE      DEVICE is the path to input device to open\n"
...
static void print_abs_bits(struct libevdev *dev, int axis)
{
	...
	s_abs_vals[axis] = abs->value;
	s_abs_map[axis] = make_pair(abs->minimum, abs->maximum);
}
	...
	switch (ev.type) {
		...
		case EV_SYN:
			if (ev.code == SYN_REPORT) {
				...
				int32_t x = (s_abs_vals[ABS_X] - min_x) * fb->width() / (max_x - min_x);
				int32_t y = (s_abs_vals[ABS_Y] - min_y) * fb->height() / (max_y - min_y);
...
int main(int argc, char **argv)
{
	...
	OptionSet optionset = {
		Option("|ddevice=", [&drm_dev_path](string s)
		{
			drm_dev_path = s;
		}),
		Option("|idevice=", [&input_dev_path](string s)
		{
			input_dev_path = s;
		}),
