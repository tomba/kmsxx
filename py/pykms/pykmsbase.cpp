	...
	py::class_<Framebuffer>(m, "Framebuffer")
		...
		.def("map", [](Framebuffer& self, uint32_t plane) {
			...
			if (plane >= format_info.num_planes)
				throw runtime_error("bad plane");

			auto shape = { self.height(), self.width() * format_info.planes[plane].bitspp / 8 };
			auto strides = std::array<size_t, 2>{self.stride(plane), sizeof(uint8_t)};
