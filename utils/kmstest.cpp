...
static void parse_props(Card& card, const string& prop_str, vector<PropInfo> &props, const DrmPropObject* propobj)
{
...
static vector<OutputInfo> setups_to_outputs(Card& card, ResourceManager& resman, const vector<Arg>& output_args)
{
	...
	for (auto& arg : output_args) {
		...
		case ArgType::Property:
		{
			...
			if (current_plane)
				parse_props(card, arg.arg, current_plane->props,
					    current_plane->plane);
			else if (current_output->crtc)
				parse_props(card, arg.arg,
